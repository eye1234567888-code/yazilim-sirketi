...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* ========== STAR BACKGROUND ========== */
        .star-bg { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: radial-gradient(ellipse at bottom, #1b2735 0%, #090a0f 100%); z-index: -1; overflow: hidden; pointer-events: none; }
        ...
        /* ===== HEADER ===== */
        .header { position: fixed; top: 0; width: 100%; z-index: 1000; background: rgba(10,14,26,0.85); backdrop-filter: blur(30px); padding: 10px 0; border-bottom: 1px solid rgba(255,255,255,0.03); transition: all 0.3s ease; }
        .header.scrolled { background: rgba(10,14,26,0.98); box-shadow: 0 4px 40px rgba(0,0,0,0.5); padding: 5px 0; }
        .nav-container { display: flex; justify-content: space-between; align-items: center; }
        .nav-logo { color: #fff; text-decoration: none; font-size: 28px; font-weight: 900; display: flex; align-items: center; gap: 10px; transition: 0.3s; margin-right: 60px; }
        .nav-logo:hover { transform: scale(1.05); }
        .nav-logo span { background: linear-gradient(135deg, #ff6b6b, #ffd93d, #6bcb77, #4d96ff, #ff6b6b); background-size: 300% 300%; -webkit-background-clip: text; -webkit-text-fill-color: transparent; animation: rainbowText 4s ease infinite; }
        @keyframes rainbowText { 0% { background-position: 0% 50%; } 50% { background-position: 100% 50%; } 100% { background-position: 0% 50%; } }
        .nav-logo i { font-size: 32px; color: #ff6b6b; animation: floatLogo 2s ease-in-out infinite; }
        ...
        .nav-toggle span { width: 28px; height: 3px; background: #fff; transition: all 0.3s ease; border-radius: 2px; }
        ...
        .nav-link { color: #e2e8f0; text-decoration: none; padding: 8px 18px; border-radius: 10px; transition: all 0.3s ease; font-weight: 500; font-size: 14px; display: flex; align-items: center; gap: 6px; position: relative; }
        .nav-link::after { content: ''; position: absolute; bottom: 0; left: 50%; width: 0; height: 3px; background: linear-gradient(90deg, #ff6b6b, #ffd93d, #6bcb77, #4d96ff); transition: all 0.3s ease; transform: translateX(-50%); border-radius: 2px; }
        .nav-link:hover::after { width: 80%; }
        .nav-link:hover { color: #fff; background: rgba(255,255,255,0.04); transform: translateY(-2px); }

        /* ===== MEGA MENU ===== */
        .nav-link .arrow { display: inline-block; transition: transform 0.3s ease; font-size: 11px; margin-left: 4px; }
        ...
        .mega-menu-content {
            ...
            min-width: 220px; background: rgba(10,14,26,0.97); backdrop-filter: blur(40px);
            padding: 15px 20px; border-radius: 16px; opacity: 0; visibility: hidden;
            transition: all 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275);
            border: 1px solid rgba(255,255,255,0.04); box-shadow: 0 30px 80px rgba(0,0,0,0.6); z-index: 100;
        }
        .mega-menu-content.active-mobile { opacity: 1 !important; visibility: visible !important; transform: translateX(-50%) translateY(0) !important; }

        .mega-menu-col ul { list-style: none; display: flex; flex-direction: column; gap: 4px; }
        .mega-menu-col ul li a { color: #94a3b8; text-decoration: none; display: flex; align-items: center; gap: 10px; transition: all 0.3s ease; font-size: 13px; padding: 6px 12px; border-radius: 8px; font-weight: 500; }
        .mega-menu-col ul li a:hover { color: #fff; background: rgba(255,107,107,0.06); transform: translateX(6px); }
        .mega-menu-col ul li a i { width: 20px; color: #ff6b6b; font-size: 14px; transition: 0.3s; }
        .mega-menu-col ul li a:hover i { color: #ffd93d; transform: scale(1.2); }

        .btn-primary { background: linear-gradient(135deg, #ff6b6b, #ee5a24, #ff6b6b); background-size: 200% 200%; animation: btnGradient 3s ease infinite; color: #fff !important; padding: 10px 28px !important; border-radius: 50px !important; text-decoration: none; font-weight: 600; transition: all 0.3s ease; border: none; cursor: pointer; display: inline-block; position: relative; overflow: hidden; font-size: 14px; }
        ...
        .btn-primary:hover { transform: translateY(-3px) scale(1.03); box-shadow: 0 10px 40px rgba(255,107,107,0.4); }
        @keyframes btnGradient { 0% { background-position: 0% 50%; } 50% { background-position: 100% 50%; } 100% { background-position: 0% 50%; } }

        /* ===== HERO ===== */
        .hero { min-height: 100vh; display: flex; align-items: center; padding-top: 80px; position: relative; overflow: hidden; background: transparent !important; z-index: 1; }
        .hero::before { content: ''; position: absolute; top: -40%; right: -20%; width: 600px; height: 600px; background: radial-gradient(circle, rgba(255,107,107,0.08), transparent 70%); border-radius: 50%; animation: floatBubble 8s ease-in-out infinite; pointer-events: none; }
        .hero::after { content: ''; position: absolute; bottom: -30%; left: -10%; width: 500px; height: 500px; background: radial-gradient(circle, rgba(77,150,255,0.05), transparent 70%); border-radius: 50%; animation: floatBubble 10s ease-in-out infinite reverse; pointer-events: none; }
        ...
        .hero-content .badge { display: inline-block; background: linear-gradient(135deg, rgba(255,107,107,0.15), rgba(255,217,61,0.15)); padding: 8px 22px; border-radius: 50px; font-size: 13px; color: #ffd93d; margin-bottom: 20px; border: 1px solid rgba(255,217,61,0.1); animation: fadeInUp 1s ease; letter-spacing: 1px; }
        .hero-content .badge i { margin-right: 6px; animation: pulse 2s ease-in-out infinite; }
        .hero-content h1 { font-size: 72px; font-weight: 900; line-height: 1.05; margin-bottom: 20px; animation: fadeInUp 1s ease 0.2s both; }
        .hero-content h1 span { background: linear-gradient(135deg, #ff6b6b, #ffd93d, #6bcb77, #4d96ff, #ff6b6b); background-size: 300% 300%; -webkit-background-clip: text; -webkit-text-fill-color: transparent; animation: rainbowText 4s ease infinite; }
        .hero-content p { font-size: 20px; color: #94a3b8; margin-bottom: 30px; line-height: 1.8; animation: fadeInUp 1s ease 0.4s both; }
        .hero-buttons { display: flex; gap: 15px; flex-wrap: wrap; animation: fadeInUp 1s ease 0.6s both; }
        .hero-stats { display: flex; gap: 50px; margin-top: 50px; animation: fadeInUp 1s ease 0.8s both; }
        .hero-stats .stat { text-align: center; }
        .hero-stats .stat h3 { font-size: 42px; font-weight: 800; background: linear-gradient(135deg, #ff6b6b, #ffd93d, #6bcb77, #4d96ff); background-size: 300% 300%; -webkit-background-clip: text; -webkit-text-fill-color: transparent; animation: rainbowText 4s ease infinite; }
        .hero-stats .stat p { font-size: 14px; color: #64748b; margin: 0; letter-spacing: 1px; }
        @keyframes fadeInUp { from { opacity: 0; transform: translateY(50px); } to { opacity: 1; transform: translateY(0); } }

        .hero-image { position: absolute; right: 0; top: 50%; transform: translateY(-50%); width: 40%; max-width: 480px; z-index: 1; }
        .hero-image .hero-rotator { width: 100%; aspect-ratio: 1; border-radius: 50%; background: linear-gradient(135deg, rgba(255,107,107,0.05), rgba(77,150,255,0.05)); border: 2px solid rgba(255,255,255,0.02); display: flex; align-items: center; justify-content: center; animation: rotateIn 20s linear infinite; position: relative; }
        .hero-image .hero-rotator::before { content: ''; position: absolute; width: 85%; height: 85%; border-radius: 50%; border: 2px dashed rgba(255,107,107,0.08); animation: rotateIn 30s linear infinite reverse; }
        .hero-image .hero-rotator::after { content: ''; position: absolute; width: 70%; height: 70%; border-radius: 50%; border: 2px dashed rgba(77,150,255,0.05); animation: rotateIn 40s linear infinite; }
        .hero-image .hero-rotator .center-icon { font-size: 90px; color: #ff6b6b; animation: pulse 2s ease-in-out infinite; }
        @keyframes rotateIn { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }

        /* ===== GLASS CARDS ===== */
        .glass { background: rgba(80,85,95,0.08); backdrop-filter: blur(20px); border: 1px solid rgba(200,200,200,0.05); border-radius: 20px; padding: 35px; transition: all 0.6s cubic-bezier(0.175,0.885,0.32,1.275); position: relative; overflow: hidden; cursor: pointer; }
        .glass::before { content: ''; position: absolute; top: -50%; left: -50%; width: 200%; height: 200%; background: radial-gradient(circle at var(--mouse-x,50%) var(--mouse-y,50%), rgba(200,200,200,0.03), transparent 50%); opacity: 0; transition: 0.5s; }
        .glass:hover::before { opacity: 1; }
        .glass:hover { transform: translateY(-15px) scale(1.03); border-color: rgba(200,200,200,0.12); box-shadow: 0 20px 60px rgba(0,0,0,0.3); background: rgba(80,85,95,0.12); }
        ...
        /* ===== CTA ===== */
        #cta { background: linear-gradient(135deg, #ff6b6b, #ee5a24, #ff6b6b) !important; background-size: 200% 200% !important; animation: btnGradient 4s ease infinite !important; border-top: none !important; border-bottom: none !important; }
        .cta-content { text-align: center; padding: 60px 40px; position: relative; z-index: 1; }
        .cta-content h2 { font-size: 48px; margin-bottom: 20px; color: #fff; }
        .cta-content p { font-size: 18px; margin-bottom: 30px; opacity: 0.8; max-width: 600px; margin-left: auto; margin-right: auto; color: #e2e8f0; }

        /* ===== FOOTER ===== */
        .footer { background: rgba(6,10,22,0.8); backdrop-filter: blur(20px); padding: 70px 0 20px; border-top: 1px solid rgba(255,255,255,0.02); position: relative; z-index: 1; }
        .footer-grid { display: grid; grid-template-columns: 2fr 1fr 1fr 1.5fr; gap: 40px; margin-bottom: 40px; }
        .footer-logo { font-size: 24px; font-weight: 700; margin-bottom: 15px; display: block; text-decoration: none; color: #fff; }
        .footer-logo span { background: linear-gradient(135deg, #ff6b6b, #ffd93d, #6bcb77, #4d96ff); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
        .footer-col p { color: #64748b; line-height: 1.8; margin-bottom: 20px; font-size: 14px; }
        ...
        /* ===== RESPONSIVE ===== */
        @media (max-width: 1200px) { .mega-menu-content { min-width: 180px; } .hero-content h1 { font-size: 52px; } .section-padding { padding: 80px 0; } }
        @media (max-width: 992px) { .hero-content h1 { font-size: 42px; } .hero-image { display: none; } .footer-grid { grid-template-columns: 1fr 1fr; } .section-padding { padding: 70px 0; } .section-header h2 { font-size: 36px; } .hero-stats { gap: 30px; flex-wrap: wrap; } }
        @media (max-width: 768px) {
            .nav-toggle { display: flex; }
            .nav-menu { position: fixed; top: 0; right: -100%; width: 85%; height: 100vh; background: #0a0e1a; padding: 80px 25px 30px; transition: all 0.4s ease; overflow-y: auto; z-index: 999; display: block; }
            .nav-menu.active { right: 0; }
            .nav-list { flex-direction: column; align-items: stretch; gap: 5px; }
            .nav-link { padding: 12px 18px; font-size: 16px; }
            ...
            .nav-item-mega .mega-menu-content {
                ...
                min-width: auto !important; padding: 12px 15px !important; margin-top: 5px !important;
                background: rgba(255,255,255,0.03) !important; border-radius: 12px !important;
                border: 1px solid rgba(255,255,255,0.03) !important; display: none !important;
                box-shadow: none !important; left: auto !important; top: auto !important;
            }
            .nav-item-mega .mega-menu-content.active-mobile { display: block !important; }
        ...
        @media (max-width: 480px) { .hero-content h1 { font-size: 26px; } .nav-logo { font-size: 18px; } .nav-logo i { font-size: 20px; } .section-padding { padding: 35px 0; } .section-header h2 { font-size: 22px; } .cta-content h2 { font-size: 22px; } .footer { padding: 30px 0 15px; } }
    </style>
</head>
<body>
    <div class="star-bg"><div id="stars"></div><div id="stars2"></div><div id="stars3"></div></div>

    <div id="loader"><div class="loader-ring"></div><div class="loader-text">✦ YÜKLENİYOR ✦</div></div>
        ...
        <div class="container">
            <div class="nav-container">
                ...
                <div class="nav-menu" id="navMenu">
                    <ul class="nav-list">
                        <li class="nav-item"><a href="{{ route('home') }}" class="nav-link {{ request()->routeIs('home') ? 'active' : '' }}">Ana Sayfa</a></li>
                        <li class="nav-item"><a href="{{ route('about') }}" class="nav-link {{ request()->routeIs('about') ? 'active' : '' }}">Hakkımızda</a></li>

                        <!-- ===== MEGA MENU - 1. TIK MENÜ AÇ, 2. TIK SAYFAYA GİT ===== -->
                        <li class="nav-item nav-item-mega">
                            <a href="{{ route('services') }}" class="nav-link {{ request()->routeIs('services') ? 'active' : '' }} nav-toggle-mega" id="megaToggle">
                        ...
                        <li class="nav-item"><a href="{{ route('references') }}" class="nav-link {{ request()->routeIs('references') ? 'active' : '' }}">Referanslarımız</a></li>
                        <li class="nav-item"><a href="{{ route('projects') }}" class="nav-link {{ request()->routeIs('projects') ? 'active' : '' }}">Projelerimiz</a></li>
                        <li class="nav-item"><a href="{{ route('blog') }}" class="nav-link {{ request()->routeIs('blog') ? 'active' : '' }}">Blog</a></li>
                        <li class="nav-item"><a href="{{ route('news') }}" class="nav-link {{ request()->routeIs('news') ? 'active' : '' }}">Haberlerimiz</a></li>
                        <li class="nav-item"><a href="{{ route('contact') }}" class="nav-link btn-primary {{ request()->routeIs('contact') ? 'active' : '' }}">İletişim</a></li>
                    </ul>
                </div>
            </div>
        </div>
    ...
    <footer class="footer">
        <div class="container">
            ...
            <div class="footer-bottom">
                <p>&copy; 2025 Yazılım Şirketi. Tüm hakları saklıdır.</p>
    ...
    <script>
        ...
        // Menü linklerine tıklanınca kapat (mega toggle hariç)
        ...
        // ========================================
        // MOBIL MEGA MENU TOGGLE (1. TIK MENÜ AÇ, 2. TIK SAYFAYA GİT)
        // ========================================
        (function() {
            ...
            function toggleMegaMenu(e) {
                // Desktop'ta normal link gibi çalışsın
                if (!isMobile) {
                    return true;
                }
                ...
                if (isOpen) {
                    // Menü açıkken tıklanırsa → SAYFAYA GİT
                    const href = megaToggle.getAttribute('href');
                    ...
                    // Menü kapalıyken tıklanırsa → MENÜYÜ AÇ
            ...
            // Click event
            megaToggle.addEventListener('click', toggleMegaMenu);
            ...
            // ========================================
            // MEGA MENÜ DIŞINA TIKLAYINCA KAPAT
            // ========================================
            ...
            // ========================================
            // MEGA MENÜ LİNKLERİNE TIKLAYINCA KAPAT
            // ========================================
        ...
        document.querySelectorAll('.tilt').forEach(card => {
            card.addEventListener('mousemove', function(e) {
                ...
                this.style.transform = `perspective(1000px) rotateX(${(y - centerY) / 10}deg) rotateY(${(centerX - x) / 10}deg) scale(1.03)`;
        ...
        console.log('🔥 Mobil: 1. tık menü aç, 2. tık sayfaya git!');
    </script>
    @stack('scripts')
</body>
</html>
