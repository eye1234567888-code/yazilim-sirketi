...
@section('title', $news['title'] ?? 'Haber Detay')
...
<section class="hero" style="min-height:40vh;background:transparent;">
    ...
    <div class="container">
        <div class="hero-content" data-aos="fade-up">
            <div class="badge" style="background:{{ $news['color'] }}20;color:{{ $news['color'] }};border-color:{{ $news['color'] }}30;">
                <i class="fas {{ $news['icon'] }}" style="color:{{ $news['color'] }};"></i> {{ $news['category'] }}
            </div>
            <h1 style="font-size:42px;line-height:1.2;">{{ $news['title'] }}</h1>
            <div style="display:flex;gap:20px;flex-wrap:wrap;margin-top:15px;color:#94a3b8;font-size:14px;">
                <span><i class="far fa-calendar" style="color:{{ $news['color'] }};"></i> {{ $news['date'] }}</span>
                <span><i class="fas fa-newspaper" style="color:{{ $news['color'] }};"></i> {{ $news['source'] }}</span>
            </div>
        </div>
    </div>
</section>
    ...
    <div class="container">
        <div style="max-width:900px;margin:0 auto;">
            <div class="glass" style="padding:50px;border-color:rgba(255,255,255,0.05);" data-aos="fade-up">
                <div style="font-size:60px;color:{{ $news['color'] }};margin-bottom:30px;text-align:center;opacity:0.3;">
                ...
                <div style="margin-top:40px;padding-top:30px;border-top:1px solid rgba(255,255,255,0.05);display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:15px;">
                    <a href="{{ route('news') }}" style="color:{{ $news['color'] }};text-decoration:none;font-weight:600;display:inline-flex;align-items:center;gap:8px;transition:0.3s;" onmouseover="this.style.gap='15px'" onmouseout="this.style.gap='8px'">
